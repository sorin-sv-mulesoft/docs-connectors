...
| TLS Configuration a| <<Tls>> | Defines a TLS configuration, which can be used from both the client and server sides to secure communication for the Mule app. Security protocol to use for communication. Valid values are PLAINTEXT / SSL / SASL_PLAINTEXT / SASL_SSL. Default value is PLAINTEXT or SASL_PLAINTEXT for sasl authentication(kerberos/scram/plain). If SSL or SASL_SSL was configured as protocol then the user needs to configure at least the keystore in the 'tls:context' child element of this config. | |
